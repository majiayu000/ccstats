...
mod tests {
    ...
    fn period_csv_daily_no_cost() {
        ...
        let csv = output_period_csv(&day_stats, Period::Day, &db, SortOrder::Asc, false);
    ...
    fn period_csv_daily_with_cost() {
        ...
        let db = PricingDb::default();
        let csv = output_period_csv(&day_stats, Period::Day, &db, SortOrder::Asc, true);
    ...
    fn period_csv_sort_desc() {
        ...
        let db = PricingDb::default();
        let csv = output_period_csv(&day_stats, Period::Day, &db, SortOrder::Desc, false);
    ...
    fn empty_data_returns_header_only() {
        let db = PricingDb::default();
        let csv = output_period_csv(&HashMap::new(), Period::Day, &db, SortOrder::Asc, false);
        let lines: Vec<&str> = csv.lines().collect();
